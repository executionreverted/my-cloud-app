    ...
    <DrawerRoot open={openEditProfile} onOpenChange={(details) => setOpenEditProfile(details.open)} initialFocusEl={() => ref.current}>
      ...
      <DrawerContent>
        ...
        <DrawerBody>
          ...
          <Wrap gap="0" mt="4">
            {Array.from({ length: 12 }).fill("1").map((_: string, index: number) => (
              <Image w={"40px"} h={"40px"} boxSizing={"border-box"} border={
                getBorder(_)
              }
          ...
          <Stack gap="4" mt="4">
            <Field.Root>
              ...
              <Input defaultValue={profile?.name} placeholder="P2P Network Username" onChange={(e) => setName(e.target.value)} />
            </Field.Root>
            <Field.Root>
              ...
              <Textarea defaultValue={profile?.status} placeholder="Status" onChange={(e) => setStatus(e.target.value)} />
            </Field.Root>
          </Stack>
        </DrawerBody>
        <DrawerFooter>
